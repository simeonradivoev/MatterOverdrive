...
public abstract class RenderBeam<T extends EntityLivingBase> implements IWorldLastRenderer
{
    ...
    protected abstract float getBeamTickness(T caster);
    ...
    protected boolean renderRaycastedBeam(Vec3 position,Vec3 direction,Vec3 offset,T caster)
    {
        ...
        {
            renderBeam(position,hit.hitVec,offset,getBeamColor(caster),getBeamTexture(caster),getBeamTickness(caster), caster);
            onBeamRender(caster);
        ...
        }else
        {
            ...
            renderBeam(position,position.addVector(direction.xCoord * maxDistance, direction.yCoord * maxDistance, direction.zCoord * maxDistance),offset,getBeamColor(caster),getBeamTexture(caster),getBeamTickness(caster), caster);
            onBeamRender(caster);
        }
        return false;
    }
