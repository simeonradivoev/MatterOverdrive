...
public class RegisterItemsFromRecipes implements Runnable {
    ...
    public void run() {
                ...
                if (itemStack != null) {
                    ...
                    if (entry == null) {
                        ...
                        if (matter > 0) {
                            ...
                            if (DEBUG) MatterOverdrive.log.debug("Could not calculate recipe for: %s", recipe.getRecipeOutput().getUnlocalizedName());
                        }
                    }else
                    {
                        if (DEBUG) MatterOverdrive.log.debug("Entry for: %s is already present", recipe.getRecipeOutput().getUnlocalizedName());
                    }
                }
            } catch (Exception e) {
		...
		MatterOverdrive.log.debug("Matter Recipe Calculation, Complete ! Took " + (System.currentTimeMillis() - startTime) + " Milliseconds. Registered total of " + count + " items");
        ...
        try
        {
			...
			MatterOverdrive.log.info("Registry saved at: " + savePath + ". Took " + (System.currentTimeMillis() - startTime) + " Milliseconds.");
        }
        catch (IOException e)
        {
			MatterOverdrive.log.error(String.format("Could not save registry to: %s", savePath), e);
        }
